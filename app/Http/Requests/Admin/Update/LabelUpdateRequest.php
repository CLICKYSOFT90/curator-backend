...
class LabelUpdateRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'email'                         => 'required|email:rfc,dns|unique:users,email,'.request('labelId').'|max:255',
        ...
        if (is_array($labelArtistEmails)) {
            foreach ($labelArtistEmails as $index => $email) {
                $rules["labelArtist.$index.artist_email"] = [
                    'required',
                    'email:rfc,dns',
                    Rule::unique('users', 'email')->ignore($userId, 'parent_id')->where(function ($query) use ($email, $index) {
